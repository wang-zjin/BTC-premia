...
new_dir = "S050401_trading_result_by_m/"
...
cluster_colors <- c("Long Call simple" = "blue", "Long Put simple" = "blue")
...
plt_HV <- dat_cluster %>%
  ...
  scale_y_continuous(limits = c(-120, 700), expand = c(0, 0)) +
...
# Save the plot
ggsave(filename = paste0(new_dir,trading_type,"simple_by_m_ttm",as.character(ttm),"_",as.character(cluster),".png"), plot = plt_HV, bg = "transparent", width = 14, height = 7, dpi = 300)

#### LV Cluster ####
cluster = "Cluster1"
dat_cluster <- filter(dat_by_m, Cluster == cluster)
# Define colors based on the cluster
cluster_colors <- c("Long Call simple" = "red", "Long Put simple" = "red")
...
# Save the plot
ggsave(filename = paste0(new_dir,trading_type,"simple_by_m_ttm",as.character(ttm),"_",as.character(cluster),".png"), plot = plt_LV, bg = "transparent", width = 14, height = 7, dpi = 300)
...
# Define colors based on the cluster
cluster_colors <- c("Long Call simple" = "black", "Long Put simple" = "black")
...
# Save the plot
ggsave(filename = paste0(new_dir,trading_type,"simple_by_m_ttm",as.character(ttm),"_",as.character(cluster),".png"), plot = plt_OA, bg = "transparent", width = 14, height = 7, dpi = 300)
...
ggsave(filename = paste0(new_dir, trading_type, "_simple_combined_ttm", as.character(ttm), ".png"), plot = combined_plot, width = 14, height = 21, dpi = 300)
