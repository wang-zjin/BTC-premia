...
# for (f in 1:nrow(files)) {
  ...
  # since for long put simple, we buy options on day t and sell options on day t+h
  ...
  if (compute_breakdown) {
    ...
    for (i in 1:length(M_groups)) {
      ...
      long_put_simple_out = list(day_by_day_res = dat %>%
      ...
      long_put_simple_by_M[[paste0(M_groups[i],"_steps",steps_ahead,"ahead")]] = long_put_simple_out$day_by_day_res %>% 
  ...
  # write_csv(overall_day_by_day_res,paste0("S03_trading_returns/LongPutsimple_overall_",
  #                                         files$weight[f],".csv"))
  
  
  
  if (compute_breakdown) {
    long_put_simple_by_M = Reduce(x = long_put_simple_by_M,f = bind_rows)
    write_csv(long_put_simple_by_M,paste0("S03_trading_returns/LongPutsimple_by_M_ttm",as.character(ttm),".csv"))
