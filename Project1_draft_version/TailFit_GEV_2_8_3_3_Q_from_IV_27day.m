...
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Moneyness");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Output");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Raw_density");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Fit_process");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Full_tail");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Full_tail_with_color");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Moneyness/Raw_density");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Moneyness/Fit_process");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Moneyness/Full_tail");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Moneyness/Full_tail_with_color");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/GIF");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Q_matrix");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/probability_matrix");
[~,~,~]=mkdir("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/cutoff");
...
for i = 1:numel(dates_list)
    ...
    a = strcat("Q_from_IV/Q_from_IV_",series,"/btc_Q_", date, ".csv");
    data_q = readtable(a);
    data_q([1,end],:) = [];
...
%         data_q.cdf_m,'Initial_paras_left',[0.3128,0.0456, 0.0296],'IR',interest_rate);
    ...
    saveas(gcf,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Raw_density/Raw_density_',date,'.png'))
    ...
    ylim([0, 9])
    xticks(min(rt):0.2:max(rt))
    saveas(gcf,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Fit_process/Fit_process_',date,'.png'))
    ...
    xlim([min(rt), max(rt)])
    xticks(min(rt):0.2:max(rt))
    saveas(gcf,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Full_tail/Full_tail_',date,'.png'))
    ...
    xlim([min(rt), max(rt)])
    xticks(min(rt):0.2:max(rt))
    saveas(gcf,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Return/Full_tail_with_color/Show_tail_',date,'.png'))

    % write Q density
    output = table(rt', Q_rt','VariableNames',{'Return', 'Q_density'});
    writetable(output,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Output/Q_density_logreturn_',date,'.csv'))

end

outputtable = array2table(cutoffs,"VariableNames",{'left','right'});
writetable(outputtable,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/cutoff/cutoff.csv')
outputtable = array2table(Q_matrix,"VariableNames",string(dates_list));
writetable(outputtable,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/Q_matrix/Q_matrix.csv')
outputtable = array2table(paras_r,"VariableNames",{'xi','b','a'});
writetable(outputtable,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras/paras_right.csv')
outputtable = array2table(paras_l,"VariableNames",{'xi','b','a'});
writetable(outputtable,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras/paras_left.csv')
outputtable = array2table(probability_matrix,"VariableNames",string(dates_list));
writetable(outputtable,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/probability_matrix/probability_matrix.csv')
...
%% probability matrix
probability_matrix=table2array(readtable('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/probability_matrix/probability_matrix.csv','ReadVariableNames',true));
...
saveas(gcf,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/probability_matrix/3parts.png')
%% probability with cutoff
cutoffs=table2array(readtable('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/cutoff/cutoff.csv','ReadVariableNames',true));
x=datetime(dates_list,"Format","MMMyyyy");
y=probability_matrix(1:3,:)';
...
saveas(gcf,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/probability_matrix/3parts_with_cutoff.png')
%% Parameters matrix
paras_l=table2array(readtable('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras/paras_left.csv','ReadVariableNames',true));
...
paras_r=table2array(readtable('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras/paras_right.csv','ReadVariableNames',true));
paras_report=nan(5,3);
...
saveas(gcf,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras/coefficients.png')
...
saveas(gcf,'Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/paras/coefficients_with_problematic.png')

% %% GIF
% [~,~,~]=mkdir(strcat("Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/GIF"));
% figure;
% ax=gca;
% f=gcf;
% DelayTime=.8;
...
% i=1;
% 
% a = strcat("Compare_Q_Rookley/RND/Rookley_Q_CB_1722_1_2_0/btc_pk_", date, "_bw_0.3.csv");
% data_q = readtable(a);
% 
% ii=find(data_q.spdy==max(data_q.spdy));
% spdy1=data_q.spdy(1:ii);
...
% xlim([min(rt), max(rt)])
% xticks(min(rt):0.2:max(rt))
% title(strcat("Q-density with fitted tails of ",date))
% 
% pause(DelayTime)
% F=getframe(f);
% [imind,cm]=rgb2ind(F.cdata,256);
% imwrite(imind,cm,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/GIF/Q_',num2str(ttm),'day.gif'),'gif','Loopcount',inf,'DelayTime',DelayTime);
% 
% for i=2:numel(dates_list)
%     a = strcat("Compare_Q_Rookley/RND/Rookley_Q_CB_1722_1_2_0/btc_pk_", date, "_bw_0.3.csv");
%     data_q = readtable(a);
% 
%     ii=find(data_q.spdy==max(data_q.spdy));
...
%     end
%     if any(spdy2<0)
%         ret_ub = min(ret_ub, round(data_q.ret(find(spdy2<0,1)+ii-3),2)-0.01);
%     end
%     data_q(data_q.ret>ret_ub,:)=[];
%     if any(d1<=0)
%         ret_lb = max(ret_lb,round(data_q.ret(find(d1<=0,1,"last")+2),2)+0.01);
%     end
%     if any(spdy1<0)
%         ret_lb = max(ret_lb, round(data_q.ret(find(spdy1<0,1,"last")+2),2)+0.01);
%     end
%     data_q(data_q.ret<ret_lb,:)=[];
% 
%     %%%%%%%%%%  log return  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     title(strcat("Q-density with fitted tails of ",date))
% 
%     pause(DelayTime);saveFrame(f,DelayTime,ttm);
% end
% 
% function saveFrame(ax,DelayTime,ttm)
% F=getframe(ax);
% [imind,cm]=rgb2ind(F.cdata,256);
% imwrite(imind,cm,strcat('Q_Tail_Fit/All_Tail_2_8_3_3_Q_from_IV/GIF/Q_',num2str(ttm),'day.gif'),'gif','WriteMode','append','DelayTime',DelayTime,"TransparentColor",0);
% end
% 
% 
