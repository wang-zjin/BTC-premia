...
bootstrap_epk = function(f, out_dir, bandwidth, mon_low, mon_up){
  ...
  for(i in c(1:ngrid))
  {
    ...
    # print('X')
    W = K.h(dist, bandwidth);
    # print(dim(W))
    # print(dim(X))
    # print(class(W))
    # print(class(X))
    XW = rbind(t(X[,1] * W ), t(X[,2] * W ), t(X[,3] * W ), t(X[,4] * W ) )
    beta = t(solve(XW %*% X) %*% XW %*% volas[,1]);
  ...
  c1 = ((log(mon.grid.scaled)-mean(data[,8])*tau)/sigmas[,1]^2/sqrt(tau) + sqrt(tau)/2)
  c2 = ((log(mon.grid.scaled)-mean(data[,8])*tau)/sigmas[,1]^2/sqrt(tau) - sqrt(tau)/2)
  dgds= exp(mean(data[,8])*tau) * dnorm(d1) * c1 - mon.grid.scaled * dnorm(d2) * c2;
  
  band.limit = bands.init$L[3] * sqrt(bands.init$V[,3]) * dgds ; 
  
  SPD = new.env();
  SPD$SPD = exp(mean(data[,8])*tau) * d2fdX2  ;
  SPD$lo = SPD$SPD - abs( band.limit );
  SPD$up = SPD$SPD + abs( band.limit );
  # SPD$lo = SPD$SPD - abs(exp(mean(data[,8])*tau) * d2fdX2 * band.limit );
  # SPD$up = SPD$SPD + abs(exp(mean(data[,8])*tau) * d2fdX2 * band.limit );
  SPD=as.list(SPD);
  
  CDF = new.env()
  CDF$CDF_Moneyness = exp(mean(data[,8])*tau) * fder[,2] + 1
  CDF$CDF_return = CDF$CDF_Moneyness
  
  SPD_logreturn = new.env();
  SPD_logreturn$SPD = exp(mean(data[,8])*tau) * d2fdX2 * mon.grid.scaled;
  SPD_logreturn$lo = SPD_logreturn$SPD - abs(band.limit * mon.grid.scaled);
  SPD_logreturn$up = SPD_logreturn$SPD + abs(band.limit * mon.grid.scaled);
