...
bootstrap_epk = function(f, ts, currdate, out_dir, bandwidth, mon_low, mon_up){
  ...
  dgds= ( exp(mean(data[,8])*tau) * dnorm(d1)  - mon.grid.scaled * dnorm(d2)) * ((log(mon.grid.scaled)-mean(data[,8])*tau)/sigmas[,1]^2/sqrt(tau) + sqrt(tau)/2);
  ...
  SPD_logreturn$lo = SPD_logreturn$SPD - abs(exp(mean(data[,8])*tau) * d2fdX2 * band.limit * mon.grid.scaled);
  SPD_logreturn$up = SPD_logreturn$SPD + abs(exp(mean(data[,8])*tau) * d2fdX2 * band.limit * mon.grid.scaled);
