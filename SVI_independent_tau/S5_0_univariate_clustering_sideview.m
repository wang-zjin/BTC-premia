...
common_dates = readtable('Clustering/Tau-independent/unique/moneyness_step_0d01/univariate_clustering/common_dates_cluster.csv');
...
index2 = common_dates.Cluster == 2;

dates_Q{1,1} = dates_list(index0); % Blue
dates_Q{1,2} = dates_list(index1); % Orange
dates_Q{1,3} = dates_list(index2); % Red
...
% Assign cluster values: 1=Blue, 2=Orange, 3=Red
for i = 1:length(date_headers_datetime)
    ...
    elseif ismember(date_headers_datetime(i), dates_Q{1,2})
        cluster_labels(i) = 2; % Orange
    elseif ismember(date_headers_datetime(i), dates_Q{1,3})
        cluster_labels(i) = 3; % Red
    else
...
% Define custom colormap (1=Blue, 2=Orange, 3=Red)
cmap = [0 0 1;  % Blue (Cluster 0)
        1, 0.498, 0; % Orange (Cluster 1)
        1 0 0];  % Red (Cluster 2)

colormap(cmap); % Apply colormap
caxis([1 3]); % Ensure colormap indexing matches clusters
...
ylabel('Time');
zlabel('Q Density');
title(sprintf('Q Density Surface Side View with Cluster-Based Coloring\nHV (Blue), MV (Orange), LV (Red)'));
view(90,0)
grid on;
hold off;
...
saveas(gcf, "Clustering/Tau-independent/unique/moneyness_step_0d01/univariate_clustering/sideview.png")