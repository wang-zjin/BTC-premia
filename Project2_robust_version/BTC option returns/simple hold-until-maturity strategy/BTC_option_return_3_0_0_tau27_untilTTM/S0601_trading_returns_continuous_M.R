...
new_dir = "S0601_trading_result_by_continuous_m/"
...
for (f in 1:nrow(files)){
  ...
  # moving_average_data <- dat_by_m %>%
  #   group_by(strategy, Cluster) %>%
  #   arrange(simplereturn) %>%
  #   mutate(mov_avg = rollmean(mean_return, k = 30, fill = NA, align = 'right'))  # Adjust 'k' for the window size
  # 
  # by_m_plt_long_4 = ggplot() +
  #   geom_point(data = subset(dat_by_m, Cluster %in% c("Overall")),
  #              aes(x = simplereturn, y = mean_return, color = Cluster),
  #              alpha = 0.2, size = 1, shape = 16) +
  #   geom_line(data = subset(moving_average_data, Cluster %in% c("Overall")), 
  #             aes(x = simplereturn, y = mov_avg, group = Cluster, color = Cluster)) +
  #   facet_wrap(~ strategy, nrow = 1) +
  #   scale_color_manual(values = c("Cluster0" = "blue", "Cluster1" = "red", "Overall" = "black")) +
  #   theme_minimal() +
  #   labs(title = "Strategy Analysis by Cluster (Moving Average)", x = "Simple return (K-S)/S", y = "Mean Return")
  
  # ggsave(filename = paste0(new_dir,trading_type,"simple_by_m_",files$weight[f],".png"), plot = by_m_plt_long_4, bg = "transparent", width = 10, height = 5, dpi = 300)
