...
setwd("~/同步空间/Pricing_Kernel/EPK/BTC option return/BTC_option_return_1_3_1_tau27_untilTTM/")
...
for (f in 1:nrow(files)){
    ...
    dat_by_m = dat_by_m %>% 
      ...
      mutate(Type = factor(x = Type,levels = c("Sum traded volume","Mean return (%)","Sharpe ratio")), 
             ...
             strategy = factor(x = strategy,levels = c(paste0(trading_type," Call simple"),
                                                       paste0(trading_type," Put simple"),
    ...
    ggsave(filename = paste0("S05_trading_result_by_m/",trading_type,"simple_by_m_",files$weight[f],".png"), plot = by_m_plt_long, bg = "transparent", width = 10, height = 10, dpi = 300)
