...
[~,~,~]=mkdir("RiskPremia/moneyness/Variance_Risk_Premium/"); % Create directory for output, if it doesn't exist

% Read Bitcoin Premium data from Excel files for different TTM (Time to Maturity) values
Q_P_c0_ttm27=readtable("RiskPremia/moneyness/Bitcoin_Premium/Q_P_ePDF_backward_onlyVR_HV_differentNB_ttm27.xlsx");
Q_P_c1_ttm27=readtable("RiskPremia/moneyness/Bitcoin_Premium/Q_P_ePDF_backward_onlyVR_LV_differentNB_ttm27.xlsx");
Q_P_overall_ttm27=readtable("RiskPremia/moneyness/Bitcoin_Premium/Q_P_ePDF_backward_onlyVR_OA_differentNB_ttm27.xlsx");
...
tb_VIX_RV_VRP_HV = readtable('RiskPremia/moneyness/Variance_Risk_Premium/VRP_VIX_logRV_HV.csv');
tb_VIX_RV_VRP_LV = readtable('RiskPremia/moneyness/Variance_Risk_Premium/VRP_VIX_logRV_LV.csv');
...
% BP: Pdensity - Qdensity or Pdensity - 0
...
% BP: Pdensity - 3M T-Bill
E_Q_IR_OA=mean([IR_HV.DTB3;IR_LV.DTB3]);
E_Q_IR_HV=mean(IR_HV.DTB3);
E_Q_IR_LV=mean(IR_LV.DTB3);

BP_IR_cluster=nan(3,3);
BP_IR_cluster(1,:)=[E_P_OA,E_P_HV,E_P_LV];
BP_IR_cluster(2,:)=[E_Q_IR_OA,E_Q_IR_HV,E_Q_IR_LV];
BP_IR_cluster(3,:)=BP_IR_cluster(1,:)-BP_IR_cluster(2,:);
BP_IR_cluster = BP_IR_cluster([3,1,2],:);
clear info;
info.rnames = strvcat('.','E_P-E_Q','E_P: P density','E_Q: 3M T-bill');
info.cnames = strvcat('Overall','Cluster 0','Cluster 1');
info.fmt    = '%10.2f';
mprint(BP_IR_cluster,info)
...
saveas(gcf,"RiskPremia/moneyness/Bitcoin_Premium/PK_ePDF_backward_onlyVR_NB12_OAc0c1_ttm27_1-by-1.png")
