...
%% load data
clear,clc
[~,~,~]=mkdir("RiskPremia/moneyness/Variance_Risk_Premium/");
...
dates_list = datetime(dates, "InputFormat","uuuuMMdd");
...
%     sp1=daily_price(end:-1:1,:);
    ...
    logret_before = price2ret(sp1.Adj_Close(datenum(sp1.Date)>=datenum(dates(i),"yyyymmdd")-ttm-1 & datenum(sp1.Date)<=datenum(dates(i),"yyyymmdd")-1));
    logret_after = price2ret(sp1.Adj_Close(datenum(sp1.Date)>=datenum(dates(i),"yyyymmdd") & datenum(sp1.Date)<=datenum(dates(i),"yyyymmdd")+ttm));
...
writetable(tb_Qdensity_RV_VRP_cluster0,"RiskPremia/moneyness/Variance_Risk_Premium/VRP_Qdensity_logRV_HV.csv");
writetable(tb_Qdensity_RV_VRP_cluster1,"RiskPremia/moneyness/Variance_Risk_Premium/VRP_Qdensity_logRV_LV.csv");

%% Functions
function Q_matrix = Q_density_cluster(dates_strings,ttm)
Q_table = readtable(strcat("Q_from_pure_SVI/moneyness/Q_matrix_",num2str(ttm),"day.csv"),"VariableNamesRow",1);
Q_table_new = Q_table(2:end,:);
Q_table_new.Properties.VariableNames = ["Return",string(Q_table{1,2:end})];
